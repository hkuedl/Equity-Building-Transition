...
DECADE_START_YEAR_TO_DECADE = {v: k for k, v in DECADE_START.items()}

def decade_for_year(year: int) -> str:
    """Return the analysis decade label for a calendar year."""
    y = int(year)
    if y < 2030:
        return "2020s"
    if y < 2040:
...
def present_value_factor_for_year(year: int, r: float = 0.02, base_year: int = 2020) -> float:
    """Present-value factor for a one-time cash event in a calendar year.

    The initial 2020 retrofit payment is treated as time 0, so it is not
    discounted. A replacement in 2030 is discounted by 10 years.
    """
    if abs(float(r)) < 1e-12:
        return 1.0
    return 1.0 / ((1.0 + float(r)) ** max(int(year) - int(base_year), 0))

DEFAULT_HORIZON_START = 2020
DEFAULT_HORIZON_END = 2060


# Four directional anchors are expanded to five strengths per direction.
# Ten seeded combinations per quadrant create 40 reproducible scenarios.
BASE_COST_TREND_CASES: Dict[str, Dict[str, Dict[str, float]]] = {
    "active_up_passive_up": {
        "active": {"2020s": 1.00, "2030s": 1.20, "2040s": 1.25, "2050s": 1.30},
        "passive": {"2020s": 1.00, "2030s": 1.25, "2040s": 1.40, "2050s": 1.45},
    },
    "active_down_passive_up": {
        "active": {"2020s": 1.00, "2030s": 0.80, "2040s": 0.65, "2050s": 0.60},
        "passive": {"2020s": 1.00, "2030s": 1.25, "2040s": 1.40, "2050s": 1.45},
    },
    "active_up_passive_down": {
        "active": {"2020s": 1.00, "2030s": 1.20, "2040s": 1.25, "2050s": 1.30},
        "passive": {"2020s": 1.00, "2030s": 0.75, "2040s": 0.70, "2050s": 0.65},
    },
    "active_down_passive_down": {
        "active": {"2020s": 1.00, "2030s": 0.80, "2040s": 0.65, "2050s": 0.60},
        "passive": {"2020s": 1.00, "2030s": 0.75, "2040s": 0.70, "2050s": 0.65},
    },
}

COST_TREND_SCENARIO_SEED = 20260625
COST_TREND_LEVELS: Tuple[float, ...] = (0.50, 0.65, 0.80, 1.00, 1.10)
COST_TREND_SAMPLES_PER_QUADRANT = 10

_DIRECTIONAL_ANCHOR_CURVES: Dict[Tuple[str, str], Dict[str, float]] = {
    ("active", "up"): BASE_COST_TREND_CASES["active_up_passive_up"]["active"],
    ("active", "down"): BASE_COST_TREND_CASES["active_down_passive_up"]["active"],
    ("passive", "up"): BASE_COST_TREND_CASES["active_up_passive_up"]["passive"],
    ("passive", "down"): BASE_COST_TREND_CASES["active_up_passive_down"]["passive"],
}

_COST_TREND_QUADRANTS: Tuple[Tuple[str, str, str], ...] = (
    ("active_up_passive_up", "up", "up"),
    ("active_down_passive_up", "down", "up"),
    ("active_up_passive_down", "up", "down"),
    ("active_down_passive_down", "down", "down"),
)


def _scaled_directional_curve(component_class: str, direction: str, level: float) -> Dict[str, float]:
    """Scale one directional anchor curve by a strength level in [0, 1].

    For each future decade:
        multiplier = 1 + level * (anchor_multiplier - 1)

    Therefore level=1 reproduces the original anchor curve, while smaller
    levels produce milder up/down trajectories but keep the same direction.
    """
    anchor = _DIRECTIONAL_ANCHOR_CURVES[(str(component_class), str(direction))]
    out: Dict[str, float] = {}
    for d, v in anchor.items():
        if str(d) == "2020s":
            out[str(d)] = 1.00
        else:
            out[str(d)] = round(1.0 + float(level) * (float(v) - 1.0), 6)
    return out


def build_sampled_cost_trend_cases(
    levels: Sequence[float] = COST_TREND_LEVELS,
    samples_per_quadrant: int = COST_TREND_SAMPLES_PER_QUADRANT,
    seed: int = COST_TREND_SCENARIO_SEED,
) -> Tuple[Dict[str, Dict[str, Dict[str, float]]], pd.DataFrame]:
    """Build the 40 sampled active/passive cost-trend scenarios.

    Returns
    -------
    cases
        Dictionary directly usable by cost_multiplier_for_component().
    metadata
        One row per sampled scenario, recording the active/passive direction,
        strength level, and multipliers. This is saved by Main_Pre.py.
    """
    levels = tuple(float(x) for x in levels)
    if len(levels) != 5:
        raise ValueError("Expected exactly five trend-strength levels.")
    if not (0 < int(samples_per_quadrant) <= 25):
        raise ValueError("samples_per_quadrant must be in [1, 25].")

    cases: Dict[str, Dict[str, Dict[str, float]]] = {}
    meta_rows: List[Dict[str, object]] = []

    all_pairs = [(a_idx, p_idx) for a_idx in range(1, len(levels) + 1) for p_idx in range(1, len(levels) + 1)]

    for q_idx, (quadrant, active_dir, passive_dir) in enumerate(_COST_TREND_QUADRANTS):
        # Deterministic and Python-version-stable; do not use built-in hash().
        rng = np.random.default_rng(int(seed) + 1009 * q_idx)
        chosen_pair_positions = sorted(rng.choice(len(all_pairs), size=int(samples_per_quadrant), replace=False).tolist())

        for sample_rank, pos in enumerate(chosen_pair_positions, start=1):
            active_level_idx, passive_level_idx = all_pairs[pos]
            active_level = levels[active_level_idx - 1]
            passive_level = levels[passive_level_idx - 1]

            active_curve = _scaled_directional_curve("active", active_dir, active_level)
            passive_curve = _scaled_directional_curve("passive", passive_dir, passive_level)

            case_name = f"{quadrant}_A{active_level_idx}_P{passive_level_idx}"
            cases[case_name] = {
                "active": active_curve,
                "passive": passive_curve,
            }

            row: Dict[str, object] = {
                "Cost_Trend_Case": case_name,
                "Base_Quadrant": quadrant,
                "Active_Direction": active_dir,
                "Passive_Direction": passive_dir,
                "Active_Level_Index": active_level_idx,
                "Passive_Level_Index": passive_level_idx,
                "Active_Level": active_level,
                "Passive_Level": passive_level,
                "Sample_Rank_Within_Quadrant": sample_rank,
                "Scenario_Seed": int(seed),
            }
            for d in ("2020s", "2030s", "2040s", "2050s"):
                row[f"Active_Multiplier_{d}"] = active_curve[d]
                row[f"Passive_Multiplier_{d}"] = passive_curve[d]
            meta_rows.append(row)

    return cases, pd.DataFrame(meta_rows)


COST_TREND_CASES, COST_TREND_CASE_METADATA = build_sampled_cost_trend_cases()
DEFAULT_COST_TREND_CASE = next(iter(COST_TREND_CASES))


PASSIVE_COMPONENT_KEYWORDS = ("roof", "wall", "window", "infil")
ACTIVE_COMPONENT_KEYWORDS = ("hvac", "cchp", "elec", "pv", "battery", "rooftop")

def component_cost_class(component: object) -> str:
    name = str(component).lower()
    if any(k in name for k in ACTIVE_COMPONENT_KEYWORDS):
        return "active"
    if any(k in name for k in PASSIVE_COMPONENT_KEYWORDS):
        return "passive"
    return "passive"

def cost_multiplier_for_component(component: object, install_decade: str = "2020s", cost_trend_case: str = "active_down_passive_down") -> float:
    """Return the active/passive cost multiplier for one component.

    Backward compatibility:
    - sampled 40-scenario names are looked up in COST_TREND_CASES;
    - the original four names are still accepted via BASE_COST_TREND_CASES;
    - unknown names fall back to DEFAULT_COST_TREND_CASE.
    """
    key = str(cost_trend_case)
    if key in COST_TREND_CASES:
        case = COST_TREND_CASES[key]
    elif key in BASE_COST_TREND_CASES:
        case = BASE_COST_TREND_CASES[key]
    else:
        case = COST_TREND_CASES[DEFAULT_COST_TREND_CASE]
    cls = component_cost_class(component)
    return float(case.get(cls, {}).get(str(install_decade), 1.0))

def apply_cost_trend_to_capex_by_component(capex_by_component: Mapping[str, float], install_decade: str = "2020s", cost_trend_case: str = "active_down_passive_down") -> Dict[str, float]:
    return {c: float(v) * cost_multiplier_for_component(c, install_decade, cost_trend_case) for c, v in capex_by_component.items()}
...
def annualized_capex_for_year(
    ...
    custom_weights: Optional[Mapping[str, float]] = None,
    custom_life: Optional[Mapping[str, int]] = None,
    install_decade: str = "2020s",
    cost_trend_case: str = "active_down_passive_down",
) -> float:
    ...
    for component, component_capex in capex_by_component.items():
        ...
        if cycle_start <= year < min(cycle_start + life, horizon_end):
            # Each replacement cycle uses the cost trend of the decade in which
            # that replacement starts. The initial cycle uses install_decade.
            event_decade = install_decade if cycle_index == 0 else decade_for_year(cycle_start)
            event_capex = float(component_capex) * cost_multiplier_for_component(
                component, install_decade=event_decade, cost_trend_case=cost_trend_case
            )
            cycle_capex = event_capex * (replacement_cost_factor ** cycle_index)
...
def annualized_capex_for_decade(
    ...
    values = [
        annualized_capex_for_year(
            ...
            install_decade=install_decade,
            cost_trend_case=cost_trend_case,
        )
        for y in years
...
def capex_event_cost_for_year(
    ...
    """Cash CAPEX event in one year: initial retrofit or later replacement.

    Unlike Annualized_CAPEX_*, this returns actual cash paid in the event year.
    The initial retrofit occurs at install_year. Later component replacements occur
    when (year - install_year) is an integer multiple of the component life.
    """
    year = int(year)
    install_year = int(install_year)
    if year < install_year or year >= int(horizon_end):
        return 0.0
    ...
    for component, component_capex in capex_by_component.items():
        ...
        if year == install_year:
            cycle_index = 0
        elif year > install_year and (year - install_year) % life == 0:
            cycle_index = int((year - install_year) // life)
        else:
            continue

        event_decade = install_decade if cycle_index == 0 else decade_for_year(year)
        event_capex = float(component_capex) * cost_multiplier_for_component(
            component, install_decade=event_decade, cost_trend_case=cost_trend_case
        )
        total += event_capex * (replacement_cost_factor ** cycle_index)
...
def capex_event_cost_for_decade(
    ...
    """Total cash retrofit/replacement CAPEX occurring within one decade."""
    if decade not in DECADE_START:
        raise ValueError(f"Unknown decade: {decade}. Expected one of {list(DECADE_START)}")
    start = DECADE_START[decade]
    return float(sum(
...
def capex_event_present_value_for_decade(
    total_capex: float,
    strategy: object,
    decade: str,
    ssp: Optional[str] = None,
    ...
    install_decade: str = "2020s",
    cost_trend_case: str = "active_down_passive_down",
    base_year: int = DEFAULT_HORIZON_START,
) -> float:
    """Present value of cash retrofit/replacement CAPEX events in one decade."""
    if decade not in DECADE_START:
        raise ValueError(f"Unknown decade: {decade}. Expected one of {list(DECADE_START)}")
    start = DECADE_START[decade]
    total = 0.0
    for y in range(start, start + 10):
        if install_year <= y < horizon_end:
            cash = capex_event_cost_for_year(
                total_capex=total_capex,
                strategy=strategy,
                year=y,
                ssp=ssp,
            ...
            total += cash * present_value_factor_for_year(y, r=r, base_year=base_year)
    return float(total)
...
def add_annualized_capex_columns(
    ...
    custom_weights: Optional[Mapping[str, float]] = None,
    custom_life: Optional[Mapping[str, int]] = None,
    install_decade_col: Optional[str] = None,
    cost_trend_case: str = "active_down_passive_down",
    adjusted_total_capex_col: str = "Adjusted_Total_CAPEX_for_annualization",
    output_prefix: str = "Annualized_CAPEX",
    cash_capex_prefix: str = "RetrofitReplacement_CAPEX",
    cash_capex_pv_prefix: str = "RetrofitReplacement_CAPEX_NPV",
    ...
    def _row_install_decade(row):
        if install_decade_col and install_decade_col in row.index and pd.notna(row[install_decade_col]):
            return str(row[install_decade_col])
        return DECADE_START_YEAR_TO_DECADE.get(int(install_year), "2020s") if "DECADE_START_YEAR_TO_DECADE" in globals() else "2020s"

    def _adjusted_total_capex(row):
        comps = infer_components(row[strategy_col], ssp=ssp)
        split = split_capex_by_component(
            total_capex=row["Total_CAPEX_for_annualization"],
            components=comps,
            ssp=ssp,
            custom_weights=custom_weights,
        )
        adjusted = apply_cost_trend_to_capex_by_component(split, install_decade=_row_install_decade(row), cost_trend_case=cost_trend_case)
        return float(sum(adjusted.values()))

    out[adjusted_total_capex_col] = out.apply(_adjusted_total_capex, axis=1)
    ...
    for d in decades:
        out[f"{output_prefix}_{d}"] = out.apply(
            lambda row: annualized_capex_for_decade(
                ...
                ssp=ssp,
                r=r,
                install_year=DECADE_START.get(_row_install_decade(row), install_year),
                horizon_end=horizon_end,
                replacement_cost_factor=replacement_cost_factor,
                custom_weights=custom_weights,
                custom_life=custom_life,
                install_decade=_row_install_decade(row),
                cost_trend_case=cost_trend_case,
            ),
            axis=1,
        )
    ...
    for d in decades:
        out[f"{cash_capex_prefix}_{d}"] = out.apply(
        ...
        out[f"{cash_capex_pv_prefix}_{d}"] = out.apply(
            lambda row: capex_event_present_value_for_decade(
                total_capex=row["Total_CAPEX_for_annualization"],
                strategy=row[strategy_col],
                decade=d,
                ssp=ssp,
                ...
                install_decade=_row_install_decade(row),
                cost_trend_case=cost_trend_case,
                base_year=DEFAULT_HORIZON_START,
            ),
            axis=1,
        )

    out["Total_RetrofitReplacement_CAPEX"] = out[[f"{cash_capex_prefix}_{d}" for d in decades]].sum(axis=1)
    out["Total_RetrofitReplacement_CAPEX_NPV"] = out[[f"{cash_capex_pv_prefix}_{d}" for d in decades]].sum(axis=1)
